...
uint8_t actuatorStopped = 0;  // Flag per lo stato di stop
/* USER CODE END PV */
/* Function Prototypes */
...
int main(void)
{
   ...
   while (1)
   {
       ...
       if (actuatorStopped)
       {
           HAL_Delay(1);  // Mantieni il ritardo per consistenza
           continue;       // Salta il resto del loop
       }

       if (cheneyStokesMode)
       {
           PerformCheyneStokesSequence(); // Modalità Cheyne-Stokes
       }
       else
       {
...
void SetFrequencyFromSerialInput(void)
{
   ...
   {
       if (receivedData == '8')  // Comando di stop
       {
           actuatorStopped = 1;
           TIM2->CCR1 = 99800;  // Imposta il valore mediano come stop
       }
       else if (receivedData >= '0' && receivedData <= '6')  // Comandi normali del polmone, stato da 1 a 6
       {
           actuatorStopped = 0;  // Riattiva l'attuatore
